...
function thydzikGoogleMapHeader() {
	...
	echo  chr(13).'<script src="http://maps.google.com/maps?file=api&amp;v=2&amp;key='.$thydzikGoogleMap_googleMapKey.'" type="text/javascript"></script>'.chr(13).'<script src="'.get_bloginfo('url').'/wp-content/plugins/thydzikGoogleMap/thydzikGoogleMap.js" type="text/javascript"></script>'.chr(13);
}
...
function thydzikFindGoogleMap($content) {
	...
	if ($temp === false) {
		...
		if (!url_exists($params[0])) { //check if the initial file exists, if not try appending full blog address
			$params[0] = get_bloginfo('url').'/wp-content/plugins/thydzikGoogleMap/'.$params[0];
		}
		if (!url_exists($params[0])) { //check file exists
			return 'the file exists'.$content; //exit with no change if file doesn't exist
		}
		...
		global $post;
		$mapID = 'map-'.($post->ID);
		$temp3 = '<div id="'.$mapID.'" style="width:0px; height:0px"></div>'.chr(13).'<script type="text/javascript">makeMap("'.$mapID.'", "'.$params[0].'", "'.get_bloginfo('url').'/wp-content/plugins/thydzikGoogleMap/markers/", "'.$params[1].'px", "'.$params[2].'px");</script>';//javascript for map code
...
function thydzikGoogleMap_adminPanel() {
		add_options_page('thydzik Google map', 'thydzik Google map', 10, basename(__FILE__), 'thydzikGoogleMap_subPanel');
}
...
EOF;
	...
	Google Map API Key: <input type="text" name="map_key" size="150" value="{$thydzikGoogleMap_googleMapKey}"><br>
